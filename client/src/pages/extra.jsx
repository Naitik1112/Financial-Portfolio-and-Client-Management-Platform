...
const EditGeneral = () => {
  ...
  const [claims, setClaims] = useState(Array(0).fill({ year: "", claim: "" }));
  ...
  return (
    <Box sx={{display: 'flex',flexDirection: 'column',justifyContent: 'space-between',marginTop: '120px',padding:'40px', gap: 4,width: '100%',...containerStyles}}>
      <Typography sx={{fontSize: '2rem', fontWeight: 'bold', color: '#fff', textAlign: 'center',  marginBottom: '10px',}}>
        Update General Insurance of {clientId}
      </Typography>
      <Box sx={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-between',  gap: 4, width: '100%', }}>
        ...
        <Autocomplete sx={inputStyles} disablePortal options={typelabels} renderInput={(params) => <TextField {...params} label="Type" />} componentsProps={{
            paper: {
              sx: {
